...
if ( ! Config.PinpointId || ! Config.CognitoId ) {
  console.warn("HM Analytics: Missing configuration. \
...
const getTestsForUser = () =>
  JSON.parse(window.localStorage.getItem("_hm_tests")) || {};
const addTestForUser = test =>
  window.localStorage.setItem(
    "_hm_tests",
    JSON.stringify({ ...getTestsForUser(), ...test })
  );
...
const applyTest = (test, variantId) => {
  /**
...
// Loop over test configurations and apply instructions if relevant.
testConfig.map(test => {
  // If the test is finished then bail.
  ...
  // If we're already part of this test then move on.
  if (typeof userConfig[test.id] !== 'undefined' && userConfig[test.id] !== false) {
    applyTest(test, userConfig[test.id]);
    return;
  }
  ...
  const variantId = Math.floor(Math.random() * test.variants.length);
  addTestForUser({
    [test.id]: variantId,
  });
  applyTest(test, variantId);
});
...
const getUserTests = () =>
  Object.entries(getTestsForUser()).reduce(
    (carry, [name, value]) => ({ ...carry, [`userTest_${name}`]: value }),
    {}
  );
const getAttributes = (extra = {}) =>
  Object.entries({
    ...
    (carry, [name, value]) => ({ ...carry, [name]: value.toString() }),
    {}
  );
...
const Analytics = {
  ...
  updateEndpoint: async (endpoint = {}) => {
    ...
    const EndpointRequest = Object.entries(EndpointData)
...
document.addEventListener("click", event => {
  ...
  const attributes = getAttributes({
    ...
    ...Object.entries(el.dataset).reduce(
      (carry, [name, value]) => ({
        ...carry,
        [`elementData_${name}`]: JSON.stringify(value),
      }),
      {}
    ),
  });
